...
func (s *OAuthService) HandleGoogleCallback(ctx context.Context, code string) (*domain.User, error) {
	...
	return s.userService.FindOrCreateOAuthUser(ctx, "google", profile.ID, profile.Email, profile.Name, profile.Picture)
}
...
func (s *OAuthService) HandleGitHubCallback(ctx context.Context, code string) (*domain.User, error) {
	data := url.Values{
		"code":          {code},
		"client_id":     {os.Getenv("GITHUB_CLIENT_ID")},
